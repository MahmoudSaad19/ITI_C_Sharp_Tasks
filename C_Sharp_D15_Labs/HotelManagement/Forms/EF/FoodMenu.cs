...
namespace Hotel_Manager
{
    public partial class FoodMenu : MetroForm
    {
        ...
        private void nextButton_Click(object sender, EventArgs e)
        {
            ...
            {
                BreakfastQ = int.TryParse( breakfastQTY.Text , out int Bf) ? Bf : 0;
            }
            if (lunchCheckBox.Checked)
            {
                LunchQ = int.TryParse(lunchQTY.Text, out int l) ? l : 0;
            }
            if (dinnerCheckBox.Checked)
            {
                DinnerQ = int.TryParse(dinnerQTY.Text, out int di) ? di : 0;
            }
