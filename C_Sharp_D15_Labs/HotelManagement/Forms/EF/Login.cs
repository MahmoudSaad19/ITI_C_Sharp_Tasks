...
namespace Hotel_Manager
{
    public partial class Login : MetroForm
    {
        public Login()
        {
            InitializeComponent();
            CenterToScreen();
            loginContextManager = new LOGIN_MANAGERContext();
            loginContextManager.Frontend.Load();
            loginContextManager.Kitchen.Load();
        }
        LOGIN_MANAGERContext loginContextManager;
        ...
        public bool verifier(string tableName, string username, string password)
        {
            ...
            try
            {
                ...
                if (tableName == "frontend")
                    result = loginContextManager.Frontend.Where(l => l.UserName == username && l.PassWord == password).ToList().Count;
                else if (tableName == "kitchen")
                    result = loginContextManager.Kitchen.Where(l =>
                    l.UserName == username && l.PassWord == password).ToList().Count;
