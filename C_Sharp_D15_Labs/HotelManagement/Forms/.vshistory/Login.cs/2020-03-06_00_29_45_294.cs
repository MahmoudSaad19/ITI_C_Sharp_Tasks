        ...
        public bool verifier(string tableName, string username, string password)
        {
            ...
            try
            {
                ...
                if (tableName == "frontend")
                    result = loginContextManager.frontends.Where(l => l.user_name == username && l.pass_word == password);
                else if (tableName == "kitchen")
                    result = loginContextManager.kitchens.Where(l =>
                    l.user_name == username && l.pass_word == password);
                else
                    result = loginContextManager.frontends.Where(f => 1 == 2);
                if (result. > 0)
                    success = true;
