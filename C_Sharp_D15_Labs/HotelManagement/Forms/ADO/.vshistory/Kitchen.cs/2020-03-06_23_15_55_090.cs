...
namespace Hotel_Manager
{
    public partial class Kitchen : MetroForm
    {
        ...
        private void updateButton_Click(object sender, EventArgs e)
        {
            if (primaryID > 1000)
            {
                var res = contextFrontEndManager.reservations.Where(r => r.Id == primaryID);
                foreach (var i in res)
                {
                    i.total_bill = totalBill + foodBill;
                    i.break_fast = breakfast;
                    i.lunch = lunch;
                    i.dinner = dinner;
                    i.supply_status = supply_status;
                    i.cleaning = Convert.ToBoolean(cleaning);
                    i.towel = Convert.ToBoolean(towel);
                    i.s_surprise = Convert.ToBoolean(surprise);
                    i.food_bill = foodBill;
                }
