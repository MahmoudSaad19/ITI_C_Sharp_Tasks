...
namespace Lab
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            var zcat = ProductList.Where(p => p.UnitsInStock == 0).Distinct();
            var sprods = cat.Except(cat);

            //var grp = ProductList.Where(p => p.UnitsInStock == 0).GroupBy(c => c.Category);
            foreach (var item in sprods)
            {
                Console.WriteLine(item);
            }
            ...
            var Catproud = from c in Category
                           let Most_Expensive = (from i in ProductList
                                                where i.Category == c  ProductList.Max(p=>p.UnitPrice)
                                                select i.UnitPrice).Max()
                                            select new {cat  = c , prod = i , }
                           
                           
            foreach (var i in Catproud)
                Console.WriteLine(i);

            var dataset = from product in ProductList
                           group product by product.Category
                           into product_Group
                           let product_Price = product_Group.Max(prd => prd.UnitPrice)
                           select new { category = product_Group.Key, product = product_Group.Where(prod => prod.UnitPrice == product_Price).FirstOrDefault(), productprice = product_Price };
