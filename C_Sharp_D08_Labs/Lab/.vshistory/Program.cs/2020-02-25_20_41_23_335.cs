...
namespace Lab
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            //var res1 = numbersA.Select(i => $"{i} is less than {numbersB.Where}").Where(t => t >);
            Console.WriteLine("Pairs where a < b:");
            for (int i = 0; i < res1.Count(); i++)
            { 
                foreach (var item in res1.Skip(i).Take(1))
                {
                    Console.WriteLine($"{numbersA[i]} is less than {item}");
                }
                //Console.WriteLine();
            }
