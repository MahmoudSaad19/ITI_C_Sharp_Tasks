...
namespace Lab
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            //var seq3 = ProductList.Select(p =>string.Empty+ p.ProductName.TakeLast(3));
            var seq3 = ProductList.TakeLast(3);
            foreach (var item in seq3)
            {
                Console.WriteLine(item);
            }
            Console.WriteLine(ProductList.Select(p => p.ProductName).Last());
            foreach (var item in ProductList.Select(p => p.ProductName).Last().TakeLast(3))
            {
                Console.WriteLine(item);
