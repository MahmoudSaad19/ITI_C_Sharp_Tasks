...
namespace Lab
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            var res2 = numbersA.Select(a => numbersB.Where(b => a < b).SelectMany((d) => $"{a} is less than {d}"));
            //var res3 = numbersA.Select(a => new { val = a, iter = numbersB.Where(b => a < b) }).SelectMany(k => k.iter).Select(s=>$"{k} is less than {s}")
