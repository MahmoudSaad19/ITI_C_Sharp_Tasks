    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            foreach (var item in grp)
            {
                ...
                {
                    Console.WriteLine($"Category : {item.Key} :: {i}");
                }
            }
            Console.WriteLine();
            Console.WriteLine();
        ...
        static void lab()
        {
            ...
            var grp = ProductList.Where(p => p.UnitsInStock == 0).GroupBy(c => c.Category);
            var grp12 = ProductList.Where(p => grp.Any(g => g.Key == p.Category));
            foreach (var item in grp)
            {
                foreach (var i in item)
                {
            ...
            var grp2 = ProductList.Where(p => sprods.Contains(p.Category)).GroupBy(c => c.Category);
            foreach (var item in grp2)
            {
                foreach (var i in item)
                {
