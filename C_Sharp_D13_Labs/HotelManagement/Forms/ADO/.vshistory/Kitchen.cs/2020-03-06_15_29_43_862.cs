        ...
        private void LoadForDataGridView()
        {
            if (connection.State != ConnectionState.Open)
            {
                ...
                try
                {
                    ...
                    Trace.WriteLine(dataTable.Columns.Count);
                    contextFrontEndManager.reservations.Load();
                    var res = contextFrontEndManager.reservations.Where(r => r.check_in == true && r.supply_status == false).Select(r=> new reservation() { Id = r.Id , first_name = r.first_name , last_name = r.last_name , phone_number = r.phone_number , room_type = r.room_type  }).ToList();
                    overviewDataGridView.DataSource = /*bindingSource; */ res;
                    //Kitchen
