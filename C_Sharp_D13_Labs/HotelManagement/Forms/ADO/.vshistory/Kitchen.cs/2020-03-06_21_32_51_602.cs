        ...
        private void listBoxFromDataBase()
        {
            ...
            if (connection.State != ConnectionState.Open)
            {
                ...
                try
                {
                    ...
                    queueListBox.DataSource = contextFrontEndManager.reservations.Where(r => r.check_in == true && r.supply_status == false).Select(i => $"{i.Id} | {i.first_name} | {i.last_name} | {i.phone_number}").ToList();
                    connection.Close();
                }
                catch (Exception ex)
                {
